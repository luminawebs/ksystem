...
$('.modal').on('show.bs.modal', function () {
    modalCount++;
    $(this).css('z-index', 1040 + (modalCount * 10));
    
    // Crear un nuevo fondo oscuro para cada modal adicional
    let $backdrop = $('<div class="modal-backdrop fade show"></div>');
    $backdrop.css('z-index', 1039 + (modalCount * 10));
    $('body').append($backdrop);
});

$('.modal').on('hidden.bs.modal', function () {
    modalCount--;
    $('.modal-backdrop').last().remove();  // Remover el último fondo oscuro
    
    if (modalCount > 0) {
        $('body').addClass('modal-open');  // Mantener scroll bloqueado si hay modales abiertos
    }
});